...
class SuperAdminDatabaseController extends Controller
{
    public function index()
    {
        ...
        return view('super-admin.database.index', [
            'tableData' => $tableData,
    ...
    public function clearAll(Request $request)
    {
        ...
        try {
            ...
            return back()->with('success', 'Toutes les tables (excepte les tables systeme) ont ete supprimees avec succes.');
        } catch (\Exception $e) {
            DB::statement('SET FOREIGN_KEY_CHECKS=1;');

            return back()->with('error', 'Erreur lors de la suppression complete: ' . $e->getMessage());
        }
    }
    ...
    public function backup(): StreamedResponse
    {
        ...
        $tables = $this->getDatabaseTables();
        $fileName = 'sauvegarde-super-admin-' . now()->format('Y-m-d_H-i-s') . '.sql';

        return response()->streamDownload(function () use ($tables) {
            echo "-- Sauvegarde de la base de donnees\n";
            echo '-- Generee le ' . now()->format('Y-m-d H:i:s') . "\n\n";
    ...
    public function exportTable(Request $request): StreamedResponse
    {
        ...
        if (in_array($tableName, $excludedTables, true)) {
            abort(403, "Cette table ne peut pas etre exportee.");
        }
    ...
    protected function authorizeAnyDatabaseAccess(): void
    {
        if (!$this->hasAnyDatabasePermission([
            ...
            abort(403, 'Acces refuse. Une permission base de donnees est requise.');
        }
    }
    ...
    protected function authorizeDatabasePermission(string $permission): void
    {
        if (!$this->hasAnyDatabasePermission([$permission])) {
            abort(403, 'Acces refuse. Permission "' . $permission . '" requise.');
        }
    }
