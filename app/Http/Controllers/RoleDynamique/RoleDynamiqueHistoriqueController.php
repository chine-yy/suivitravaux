...
class RoleDynamiqueHistoriqueController extends Controller
{
    ...
    private function hasPermission(string $permission): bool
    {
        ...
        $aliases = [
            'view-historique',
            'view-historique-budgets',
        ];

        return $user->role->permissions()->whereIn('slug', $aliases)->exists();
    }
    ...
    {
        if (!$this->hasPermission('view-historique')) {
            abort(403, 'Accès refusé. Permission "view-historique" requise.');
        }
        ...
        return view('role-dynamique.historique.index', compact('annees', 'statsParAnnee'));
    }
    ...
    public function show($annee)
    {
        ...
        return view('role-dynamique.historique.show', compact(
    ...
    {
        $this->hasPermission('view-historique') ?: abort(403);
        $annee = (int) $annee;

        // Récupérer les mêmes données que show()
    ...
    {
        // Copier la logique de show() ici et retourner les données
        $projets = Projet::with([
        ...
        $rolesRaw = Role::withCount('users')->with('permissions')->whereNotIn('nom', ['Administration', 'Super Admin', 'Partenaire'])->get();
        ...
        $admins = \App\Models\User::with('role')->entrepriseAdmins()->whereYear('created_at', '<=', $annee)->get();
