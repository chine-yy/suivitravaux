...
<body>
    <div class="container">
        <div class="header">
            <h1>Sécurité {{ config('app.name') }}</h1>
        </div>
        <div class="content">
            <p>Bonjour {{ $user->name ?? $user->nom }},</p>
            <p>Nous vous informons que le mot de passe de votre compte {{ config('app.name') }} a été modifié avec succès.</p>
        ...
        <div class="footer">
            &copy; {{ date('Y') }} {{ config('app.name') }} - Système de Suivi des Travaux. Tous droits réservés.
        </div>
    </div>
</body>
</html>
