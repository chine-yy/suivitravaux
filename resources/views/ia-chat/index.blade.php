...
@section('title', 'IA Chat Box - Assistant')
        ...
        <div class="ia-chat-sidebar">
            ...
            <div class="flex-grow-1 overflow-auto">
                @forelse($conversations as $conversation)
                    <div class="conversation-item {{ $selectedConversation == (is_object($conversation) ? $conversation->id : $conversation['id']) ? 'active' : '' }}" 
                         onclick="selectConversation({{ is_object($conversation) ? $conversation->id : $conversation['id'] }})">
                        <div class="d-flex justify-content-between align-items-start">
                            <div class="flex-grow-1 overflow-hidden">
                                <div class="fw-bold small text-truncate">{{ is_object($conversation) ? $conversation->preview : ($conversation['preview'] ?? 'Nouvelle conversation') }}</div>
                                <small class="text-muted">{{ \Carbon\Carbon::parse(is_object($conversation) ? $conversation->updated_at : $conversation['updated_at'])->diffForHumans() }}</small>
                            </div>
                            <button class="btn btn-sm text-danger p-0 ms-2" onclick="event.stopPropagation(); deleteConversation({{ is_object($conversation) ? $conversation->id : $conversation['id'] }})" title="Supprimer">
                    ...
                    @forelse($messages as $message)
                        <div class="message-bubble {{ $message->role === 'user' ? 'message-user' : 'message-assistant' }}">
                            @if($message->image_path)
                                <img src="{{ asset('storage/' . $message->image_path) }}" class="img-fluid rounded mb-2" style="max-width: 200px;" alt="Image">
                            @endif
...
<script>
let currentConversationId = {{ $selectedConversation ? $selectedConversation : 'null' }};

function createNewConversation() {
    fetch('{{ route($baseRoute . '.create') }}', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (data.success) {
            window.location.href = '{{ route($baseRoute . '.index') }}?conversation_id=' + data.conversation_id;
        }
    });
}

function selectConversation(id) {
    window.location.href = '{{ route($baseRoute . '.index') }}?conversation_id=' + id;
}
...
function deleteConversation(id) {
    ...
    fetch('{{ route($baseRoute . '.destroy', ['id' => '__ID__']) }}'.replace('__ID__', id), {
        method: 'DELETE',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Content-Type': 'application/json'
    ...
    .then(data => {
        if (data.success) {
            window.location.href = '{{ route($baseRoute . '.index') }}';
        }
    });
}
