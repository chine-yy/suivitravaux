...
@section('breadcrumb')
    <a href="{{ route('role-dynamique.configuration.index') }}" class="text-muted">Configuration</a>
    ...
    <div class="cp-content">
        <div class="cp-page-header">
            ...
            <div class="d-flex gap-2">
                @if($canClear)
                <form action="{{ route('role-dynamique.configuration.logs.clear') }}" method="POST" onsubmit="return confirm('Voulez-vous vraiment vider le fichier de logs ?')">
                    @csrf
                ...
                @endif
                @if($canExport)
                <a href="{{ route('role-dynamique.configuration.logs.export-pdf') }}" class="btn btn-outline-danger rounded-pill px-3">
        ...
        <div class="cp-card">
            ...
            <div class="cp-card-body p-0">
                <pre class="bg-dark text-light p-4 mb-0" style="max-height: 600px; overflow-y: auto; font-family: 'Courier New', Courier, monospace; font-size: 0.85rem;">{{ $logs ?: 'Aucun log disponible pour le moment.' }}</pre>
            </div>
        </div>
    </div>
</div>
