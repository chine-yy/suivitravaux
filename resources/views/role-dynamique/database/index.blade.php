...
@section('title', 'Gestion de la Base de Donnees')

@section('breadcrumb')
    <a href="{{ route('role-dynamique.configuration.index') }}" class="text-decoration-none">Configuration</a>
    ...
    <div class="cp-content">
        ...
        <div class="cp-chart-card mb-4">
            ...
            <div class="p-4">
                ...
                <a href="{{ route('role-dynamique.database.backup') }}" class="btn btn-primary">
        ...
        @if($canViewDatabase || $canClearDatabase)
                    ...
                    <table class="table table-hover align-middle">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="badge {{ $table['count'] > 0 ? 'bg-primary' : 'bg-secondary' }}">
                                        {{ $table['count'] }}
                                    </span>
                                </td>
                                <td class="text-end">
                                    @if($canViewDatabase)
                                    <button type="button" class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#viewModal{{ $table['name'] }}">
                                        <i class="bi bi-eye"></i> Voir
                                    </button>
                                    @endif

                                    @if($canExportDatabase)
                                    <a href="{{ route('role-dynamique.database.export-table', ['table' => $table['name']]) }}" class="btn btn-sm btn-outline-success">
                                        <i class="bi bi-download"></i> Exporter
                                    </a>
                                    @endif

                                    @if($canClearDatabase)
                                    <button type="button" class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#clearModal{{ $table['name'] }}" {{ $table['count'] == 0 ? 'disabled' : '' }}>
                                        <i class="bi bi-trash"></i> Vider
                                    </button>

                                    <div class="modal fade" id="clearModal{{ $table['name'] }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-hidden="true" style="background: rgba(0,0,0,0.5);">
                                        <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content" style="border-radius: 16px;">
                                                ...
                                                <div class="modal-body text-center pt-4 pb-4">
                                                    <p class="mb-3">Êtes-vous sûr de vouloir vider le contenu de la table <code>{{ $table['name'] }}</code> ?</p>
                                                ...
                                                <div class="modal-footer border-0 pt-0 justify-content-center gap-3">
                                                    ...
                                                    <form action="{{ route('role-dynamique.database.clear-table') }}" method="POST" class="d-inline">
                                                        @csrf
                                    ...
                                    @if($canViewDatabase)
                                    <div class="modal fade" id="viewModal{{ $table['name'] }}" tabindex="-1" aria-hidden="true">
                                        <div class="modal-dialog modal-xl">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title">Table: {{ $table['name'] }}</h5>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                </div>
                                                    ...
                                                    <div class="table-responsive">
                                                        <table class="table table-sm table-bordered" style="max-height: 50vh;">
                                                            ...
                                                            <tbody>
                                                                ...
                                                                @empty
                                                                <tr>
                                                                    <td colspan="{{ count($columns) }}" class="text-center text-muted">Aucun enregistrement</td>
                                                                </tr>
                                                                @endforelse
                                                            </tbody>
                                                        </table>
                                                ...
                                                <div class="modal-footer">
                                                    @if($canExportDatabase)
                                                    <a href="{{ route('role-dynamique.database.export-table', ['table' => $table['name']]) }}" class="btn btn-success">
                                                        <i class="bi bi-download me-1"></i> Exporter
                                                    </a>
                                                    @endif
                                                    @if($canClearDatabase && $table['count'] > 0)
                                                    <button type="button" class="btn btn-danger"
                                                        onclick="var vm=bootstrap.Modal.getInstance(document.getElementById('viewModal{{ $table['name'] }}')); vm.hide(); document.getElementById('viewModal{{ $table['name'] }}').addEventListener('hidden.bs.modal', function handler(){ this.removeEventListener('hidden.bs.modal', handler); new bootstrap.Modal(document.getElementById('clearModal{{ $table['name'] }}')).show(); });">
...
<div id="clearAllContainer" class="clearAll-container" style="display: none;">
    <div class="clearAll-box" style="max-width: 520px; width: calc(100% - 32px); background: #fff; border-radius: 20px; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.25);">
        ...
        <div class="d-flex justify-content-center align-items-center gap-3 px-4 pb-4 pt-2" style="border: none;">
            ...
            <form action="{{ route('role-dynamique.database.clear-all') }}" method="POST" style="margin: 0;">
                @csrf
