@php
    $routePrefix = $routePrefix ?? 'role-dynamique';
...
<div class="cp-card mb-4" id="depenses">
    <div class="cp-card-header d-flex justify-content-between align-items-center">
        ...
        <div class="d-flex gap-2">
            @if($canPermission('exporter-pdf-depenses'))
            <button class="btn btn-sm btn-outline-danger" onclick="exportToPdf('depensesTable', 'Gestion dépenses', 'depenses_export')">
                <i class="bi bi-file-earmark-pdf me-1"></i>Exporter tout
            </button>
            @endif
            @if($canPermission('create-depenses'))
                            ...
                            <tr>
                                ...
                                <td class="text-end">
                                    <div class="d-flex gap-1 justify-content-end align-items-center">
                                        ...
                                        @if($canPermission('edit-depenses'))
                                        ...
                                        @endif
                                        @if($canPermission('exporter-pdf-depenses'))
                                        <a href="{{ route($routePrefix . '.export.pdf.direct', ['type' => 'depense', 'id' => $depense->id]) }}"
                                            class="btn btn-sm btn-outline-secondary" title="Télécharger">
                                            <i class="bi bi-download"></i>
                                        </a>
                                        @endif
                                        @if($canPermission('delete-depenses'))
                                            <form action="{{ route($routePrefix . '.budget.depenses.destroy', $depense) }}" method="POST"
                                                class="d-inline-flex m-0" onsubmit="return confirm('Supprimer cette dépense ?')">
                                                @csrf
                                                @method('DELETE')
            ...
            <form action="{{ route($routePrefix . '.budget.depenses.store') }}" method="POST">
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            <label class="form-label">Montant (FCF) <span class="text-danger">*</span></label>
