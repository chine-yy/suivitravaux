        ...
        <div class="cp-content">
            <div class="cp-page-header">
                ...
                <div class="d-flex gap-2">
                    @if($canPermission('exporter-pdf-projets'))
                    ...
                    @endif
                    @if($canPermission('create-projets'))
                        <a href="{{ route('role-dynamique.projets.create') }}" class="btn btn-primary px-4">
            ...
            <div class="cp-stats-grid mb-4">
                ...
                <div class="cp-stat-card">
                    ...
                    <div class="cp-stat-content">
                        <div class="cp-stat-value">{{ $enCours }}</div>
                ...
                <div class="cp-stat-card">
                    ...
                    <div class="cp-stat-content">
                        <div class="cp-stat-value">{{ $termines }}</div>
                ...
                <div class="cp-stat-card cp-stat-danger">
                    ...
                    <div class="cp-stat-content">
                        <div class="cp-stat-value">{{ $enRetard }}</div>
            ...
            <div class="cp-chart-card mb-4">
                ...
                <div class="p-4">
                    <form action="{{ route('role-dynamique.projets.index') }}" method="GET" class="row g-3">
                        <div class="col-md-4">
                            <label class="form-label small fw-bold">Nom du Projet</label>
                            <input type="text" name="nom" class="form-control form-control-sm"
                                placeholder="Ex: Construction villa..." value="{{ request('nom') }}">
                        </div>
                        <div class="col-md-3">
                        ...
                        <div class="col-md-2 d-flex align-items-end gap-2">
                            ...
                            <a href="{{ route('role-dynamique.projets.index') }}" class="btn btn-sm btn-outline-secondary">
                    ...
                    <table class="table table-hover mb-0" id="projetsTable">
                        ...
                        <tbody>
                            @forelse($projets as $projet)
                                <tr>
                                    <td>
                                        <span class="fw-bold">{{ $projet->nom }}</span>
                                        @if($projet->date_fin_prevue)
                                                                <div class="small text-muted mt-1"><i class="bi bi-calendar-event me-1"></i>Échéance: {{
                                            $projet->date_fin_prevue->format('d/m/Y') }}</div>
                                        @endif
                                    </td>
                                    <td>
                                        @php
                                        ...
                                        <div class="d-flex align-items-center gap-2">
                                            <div class="progress flex-grow-1" style="height: 8px; border-radius: 4px;">
                                                <div class="progress-bar bg-primary" role="progressbar"
                                                    style="width: {{ $projet->avancement ?? 0 }}%;"></div>
                                            </div>
                                            <span class="fw-bold small" style="min-width:35px;">{{ $projet->avancement ?? 0
                                                        }}%</span>
                                        </div>
                                    </td>
                                    <td class="text-end">
                                        <div class="d-flex gap-1 justify-content-end">
                                            @if($canPermission('view-projets'))
                                                <a href="{{ route('role-dynamique.projets.show', $projet->id) }}"
                                            ...
                                            @endif
                                            @if($canPermission('edit-projets'))
                                                <a href="{{ route('role-dynamique.projets.edit', $projet->id) }}"
                                                    class="btn btn-sm btn-outline-primary" title="Modifier">
                                                    <i class="bi bi-pencil"></i>
                                                </a>
                                            @endif
                                            @if($canPermission('delete-projets'))
                                                <form action="{{ route('role-dynamique.projets.destroy', $projet->id) }}"
...
@if($canPermission('create-projets'))
                                                <a href="{{ route('role-dynamique.projets.create') }}">Créer un projet</a>
                                            @endif
                                    </td>
                                </tr>
                            @endforelse
