    ...
    <div class="cp-content">
        <div class="cp-page-header">
            ...
            <div class="d-flex gap-2">
                @if($has('exporter-pdf-rendez-vous'))
                <button class="btn btn-outline-danger" onclick="exportToPdf('rendezvousTable', 'Liste des rendezvous', 'rendezvous_export')">
                    <i class="bi bi-file-earmark-pdf me-2"></i>Exporter PDF
                </button>
                @endif
                @if($has('create-rendez-vous'))
                <a href="{{ route('role-dynamique.rendezvous.create') }}" class="btn btn-primary px-4">
        ...
        <div class="cp-chart-card mb-4">
            ...
            <div class="p-4">
                <form action="{{ route('role-dynamique.rendezvous.index') }}" method="GET" class="row g-3">
                    <div class="col-md-2">
                    ...
                    <div class="col-md-2 d-flex align-items-end gap-2">
                        ...
                        <a href="{{ route('role-dynamique.rendezvous.index') }}"
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-end">
                                <div class="d-flex gap-1 justify-content-end">
                                    ...
                                    <a href="{{ route('role-dynamique.rendezvous.show', $rdv->id) }}"
                                        class="btn btn-sm btn-outline-info" title="Voir">
                                        <i class="bi bi-eye"></i>
                                    </a>
                                    @endif
                                    @if($has('edit-rendez-vous'))
                                    <a href="{{ route('role-dynamique.rendezvous.edit', $rdv->id) }}"
                                        class="btn btn-sm btn-outline-primary" title="Modifier">
                                        <i class="bi bi-pencil"></i>
                                    </a>
                                    @endif
                                    @if($has('exporter-pdf-rendez-vous'))
                                    <a href="{{ route('role-dynamique.export.pdf.direct', ['type' => 'rendezvous', 'id' => $rdv->id]) }}" class="btn btn-sm btn-outline-secondary" title="Télécharger">
                                        <i class="bi bi-download"></i>
                                    </a>
                                    @endif
                                    @if($has('delete-rendez-vous'))
                                    <form action="{{ route('role-dynamique.rendezvous.destroy', $rdv->id) }}" method="POST"
                                        class="d-inline">
                                        @csrf @method('DELETE')
                        ...
                        <tr>
                            <td colspan="9" class="text-center py-5 text-muted">
                                ...
                                @if($has('create-rendez-vous'))
                                <a href="{{ route('role-dynamique.rendezvous.create') }}">Créer un rendez-vous</a>
                                @endif
                            </td>
                        </tr>
                        @endforelse
