    ...
    <div class="cp-content">
        <div class="cp-page-header">
            ...
            <div class="d-flex gap-2">
                ...
                @endif
                @if($has('create-factures'))
                <a href="{{ route('role-dynamique.factures.create') }}" class="btn btn-primary px-4">
        ...
        <div class="cp-chart-card mb-4">
            ...
            <div class="p-4">
                <form action="{{ route('role-dynamique.factures.index') }}" method="GET" class="row g-3">
                    <div class="col-md-3">
                    ...
                    <div class="col-md-2 d-flex align-items-end gap-2">
                        ...
                        <a href="{{ route('role-dynamique.factures.index') }}" class="btn btn-sm btn-outline-secondary">
                        ...
                        <tr>
                            ...
                            <td class="text-end">
                                <div class="d-flex gap-1 justify-content-end">
                                    <a href="{{ route('role-dynamique.factures.show', $facture->id) }}"
                                        class="btn btn-sm btn-outline-info" title="Voir">
                                        <i class="bi bi-eye"></i>
                                    </a>
                                    @if($has('exporter-pdf-factures'))
                                    <a href="{{ route('role-dynamique.export.pdf.direct', ['type' => 'facture', 'id' => $facture->id]) }}"
                                        class="btn btn-sm btn-outline-secondary" title="Télécharger PDF">
                                        <i class="bi bi-download"></i>
                                    </a>
                                    @endif
                                    @if($has('edit-factures'))
                                    <a href="{{ route('role-dynamique.factures.edit', $facture->id) }}"
                                        class="btn btn-sm btn-outline-primary" title="Modifier">
                                        <i class="bi bi-pencil"></i>
                                    </a>
                                    @endif
                                    @if($has('delete-factures'))
                                    <form action="{{ route('role-dynamique.factures.destroy', $facture->id) }}"
                                    ...
                                    @endif
                                    @if($has('envoyer-partenaire-factures'))
                                        @if(!$facture->est_envoye_partenaire)
                                        <form action="{{ route('role-dynamique.factures.envoyer-partenaire', $facture->id) }}" method="POST" class="d-inline">
                                            @csrf
...
<a href="{{ route('role-dynamique.factures.create') }}">Créer une facture</a>
@endif
                            </td>
                        </tr>
                        @endforelse
