    ...
    <div class="cp-content">
        <div class="cp-page-header">
            ...
            <div class="d-flex gap-2">
                ...
                <a href="{{ route('role-dynamique.contrats.export-pdf') }}{{ request()->getQueryString() ? '?' . request()->getQueryString() : '' }}"
                   class="btn btn-outline-danger">
                    <i class="bi bi-file-earmark-pdf me-2"></i>Exporter PDF
                </a>
                @endif
                @if($has('create-contrats'))
                <a href="{{ route('role-dynamique.contrats.create') }}" class="btn btn-primary px-4">
        ...
        <div class="cp-chart-card mb-4">
            ...
            <div class="p-4">
                <form action="{{ route('role-dynamique.contrats.index') }}" method="GET" class="row g-3">
                    <div class="col-md-4">
                    ...
                    <div class="col-md-2 d-flex align-items-end gap-2">
                        ...
                        <a href="{{ route('role-dynamique.contrats.index') }}" class="btn btn-sm btn-outline-secondary">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-end">
                                <div class="d-flex gap-1 justify-content-end">
                                    <a href="{{ route('role-dynamique.contrats.show', $contrat->id) }}"
                                        class="btn btn-sm btn-outline-info" title="Voir">
                                        <i class="bi bi-eye"></i>
                                    </a>
                                    @if($has('exporter-pdf-contrats'))
                                    <a href="{{ route('role-dynamique.export.pdf.direct', ['type' => 'contrat', 'id' => $contrat->id]) }}"
                                        class="btn btn-sm btn-outline-secondary" title="Télécharger">
                                        <i class="bi bi-download"></i>
                                    </a>
                                    @endif
                                    @if($has('envoyer-partenaire-contrats'))
                                        @if(!$contrat->est_envoye_partenaire)
                                        <form action="{{ route('role-dynamique.contrats.envoyer-partenaire', $contrat->id) }}" method="POST" class="d-inline">
                                            @csrf
                                    ...
                                    <a href="{{ route('role-dynamique.contrats.edit', $contrat->id) }}"
                                        class="btn btn-sm btn-outline-primary" title="Modifier">
                                        <i class="bi bi-pencil"></i>
                                    </a>
                                    @endif
                                    @if($has('delete-contrats'))
                                    <form action="{{ route('role-dynamique.contrats.destroy', $contrat->id) }}"
                        ...
                        <tr>
                            <td colspan="8" class="text-center py-5 text-muted">
                                ...
                                @if($has('create-contrats'))
<a href="{{ route('role-dynamique.contrats.create') }}">Créer un contrat</a>
@endif
                            </td>
                        </tr>
                        @endforelse
