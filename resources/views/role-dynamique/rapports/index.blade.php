    ...
    <div class="cp-content">
        ...
        <div class="cp-page-header">
            ...
            <div class="d-flex gap-2">
                ...
                @endif
                @if($canPermission('create-rapports'))
                <a href="{{ route('role-dynamique.rapports.create') }}" class="btn btn-green">
        ...
        <div class="cp-stats-grid mb-4" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px;">
            ...
            <div class="cp-stat-card">
                ...
                <div class="cp-stat-content">
                    <div class="cp-stat-value">{{ $totalMesRapports ?? ($mesRapports?->total() ?? 0) }}</div>
            ...
            <div class="cp-stat-card">
                ...
                <div class="cp-stat-content">
                    <div class="cp-stat-value">{{ $totalRapportsRecus ?? ($recusRapports?->total() ?? 0) }}</div>
            ...
            <div class="cp-stat-card">
                ...
                <div class="cp-stat-content">
                    <div class="cp-stat-value">{{ $totalRapports ?? ($rapports?->total() ?? 0) }}</div>
                    <div class="cp-stat-label">{{ $totalRapportsLabel ?? 'Total Rapports' }}</div>
                </div>
            </div>
            <div class="cp-stat-card">
                <div class="cp-stat-icon cp-bg-warning"><i class="bi bi-eye-slash"></i></div>
                <div class="cp-stat-content">
                    <div class="cp-stat-value">{{ $rapportsParStatut['non_lu'] ?? 0 }}</div>
                    <div class="cp-stat-label">Non lus</div>
                    <a href="{{ route('role-dynamique.rapports.index', ['filter' => 'non_lu']) }}#rapports-table-container" class="btn btn-sm btn-link p-0 text-green fw-bold" style="text-decoration: none;">Voir</a>
                </div>
            </div>
            <div class="cp-stat-card">
        ...
        <div class="row mb-4">
            ...
            <div class="col-lg-6 mb-4">
                <div class="cp-chart-card">
                    ...
                    <div class="cp-chart-body">
                        <canvas id="rapportStatutChart" data-statuts='@json($rapportsParStatut)'></canvas>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-6 mb-4">
                <div class="cp-chart-card">
                    ...
                    <div class="cp-chart-body">
                        <canvas id="rapportProjetChart" data-projets='@json($rapportsParProjet)'></canvas>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="cp-card" id="rapports-table-container">
            ...
            <div class="p-3 border-bottom bg-white">
                <ul class="nav nav-tabs">
                    <li class="nav-item"><a class="nav-link {{ $currentTab==='mes' ? 'active' : '' }}" href="{{ route('role-dynamique.rapports.index', ['tab' => 'mes']) }}#rapports-table-container" data-tab="mes">Mes rapports ({{ $mesRapports->total() ?? 0 }})</a></li>
                    <li class="nav-item"><a class="nav-link {{ $currentTab==='recus' ? 'active' : '' }}" href="{{ route('role-dynamique.rapports.index', ['tab' => 'recus']) }}#rapports-table-container" data-tab="recus">Rapports reçus ({{ $recusRapports->total() ?? 0 }})</a></li>
                </ul>
            </div>
            @endif
            <div class="cp-card-header">
                ...
                <form action="{{ route('role-dynamique.rapports.index') }}" method="GET" class="row g-2 p-3 bg-light rounded shadow-sm">
                    <div class="col-md-3">
                        <label class="form-label small fw-bold">Titre</label>
                        <input id="rapportSearch" type="text" name="titre" class="form-control form-control-sm" placeholder="Rechercher..." value="{{ request('titre') }}">
                    </div>
                    <div class="col-md-3">
                    ...
                    <div class="col-md-2 d-flex align-items-end gap-1">
                        ...
                        <a href="{{ route('role-dynamique.rapports.index') }}" class="btn btn-sm btn-outline-secondary"><i class="bi bi-arrow-counterclockwise"></i></a>
                    </div>
                </form>
                ...
                <div id="rapports-table-inner" class="mt-3">
                    ...
                    @endphp
                    @include('role-dynamique.rapports._table', ['items' => $initialItems, 'currentTab' => $currentTab])
                </div>
            </div>
...
<script>
    ...
    function initTableBindings() {
        ...
        initTableBindings();
        const currentTab = @json($currentTab ?? 'mes');
