...
@extends($layoutSelect)

@section('title', 'Messsagerie')
...
<div class="container-fluid py-0 py-md-4">
    <div class="row g-0 rounded-md-4 shadow-lg overflow-hidden bg-white chat-main-container {{ $chatMobileClass }}">
            ...
            <div class="flex-grow-1 overflow-auto bg-white custom-scrollbar" id="contactList">
                <div class="list-group list-group-flush">
                    @forelse($contacts as $item)
                        ...
                        @endphp
                        <a href="{{ route($baseRoute . '.index', ['id' => $item['model']->id, 'type' => $item['type']]) }}"
                           class="list-group-item list-group-item-action border-0 py-3 px-3 d-flex align-items-center contact-item {{ $isActive ? 'active-contact' : '' }}">
                            ...
                            <div class="flex-grow-1 overflow-hidden">
                                <div class="d-flex justify-content-between align-items-center mb-1">
                                    <h6 class="mb-0 fw-bold text-truncate {{ $item['unread_count'] > 0 ? 'text-dark' : 'text-secondary' }}">{{ $item['fullname'] }}</h6>
                                    @if($item['unread_count'] > 0)
                                        <span class="badge bg-success rounded-pill px-2">{{ $item['unread_count'] }}</span>
                                    @endif
                                </div>
                                <div class="d-flex justify-content-between">
                                    <small class="text-muted text-truncate w-75">{{ $item['label'] }}</small>
                                </div>
                            </div>
                        </a>
                    @empty
        ...
        <div class="col-md-8 d-flex flex-column bg-white chat-conversation-pane">
            @if($selectedContact)
                ...
                <div class="p-3 border-bottom d-flex align-items-center bg-white shadow-sm z-1">
                    <a href="{{ route($baseRoute . '.index') }}" class="chat-back-btn d-md-none me-2" title="Retour aux discussions">
                    ...
                    <div class="avatar bg-success text-white rounded-circle d-flex align-items-center justify-content-center me-3 shadow-sm" style="width: 45px; height: 45px; font-weight: bold;">
                        {{ strtoupper(substr(($selectedContactMeta['fullname'] ?? ($selectedContact->prenom ?? $selectedContact->name ?? '?')), 0, 1)) }}
                    </div>
                    <div class="flex-grow-1">
                        <h6 class="mb-0 fw-bold text-dark">{{ $selectedContactMeta['fullname'] ?? (($selectedContact->prenom ?? '') . ' ' . ($selectedContact->nom ?? '') ?: ($selectedContact->name ?? 'Inconnu')) }}</h6>
                        <small class="text-success"><i class="bi bi-circle-fill me-1" style="font-size: 0.5rem;"></i> {{ $selectedContactMeta['label'] ?? request('type') }}</small>
                    </div>
                </div>
                ...
                <div class="flex-grow-1 p-4 overflow-auto d-flex flex-column custom-scrollbar" style="background: #e5ddd5 url('https://user-images.githubusercontent.com/15075759/28719144-86dc0f70-73b1-11e7-911d-60d70fcded21.png') repeat;" id="messageContainer">
                    <div id="messagesList" class="d-flex flex-column w-100"
                         ...
                         data-current-user-type="{{ $currentUser['type'] }}">
                        @forelse($messages as $msg)
                            ...
                            <div class="d-flex mb-3 {{ $isMe ? 'justify-content-end' : 'justify-content-start' }}" data-id="{{ $msg->id }}">
                                <div class="message-bubble {{ $isMe ? 'bubble-me' : 'bubble-them' }} shadow-sm">
                                    @if($msg->image_path)
                                        <div class="mb-2">
                                            <img src="{{ asset('storage/' . $msg->image_path) }}" class="img-fluid rounded-3 message-img" alt="Image" style="max-height: 300px; cursor: pointer;" onclick="window.open(this.src)">
                                        </div>
                                    @endif
                                    ...
                                    @if($msg->audio_path)
                                        <div class="mb-2">
                                            <audio controls class="mw-100 message-audio" style="height: 35px;">
                                                <source src="{{ asset('storage/' . $msg->audio_path) }}" type="audio/mpeg">
                ...
                <div class="p-3 bg-light border-top chat-footer">
                    ...
                    <form action="{{ route($baseRoute . '.store') }}" method="POST" enctype="multipart/form-data" id="chatForm">
                        @csrf
                        <input type="hidden" name="receiver_id" value="{{ $selectedContact->id }}">
                        <input type="hidden" name="receiver_type" value="{{ request('type') }}">
