                            ...
                            <table class="table table-hover align-middle mb-0">
                                ...
                                <tbody>
                                    @forelse($rapports as $rapport)
                                        <tr>
                                            <td class="px-4 py-4">{{ $rapport->created_at->format('d/m/Y') }}</td>
                                            <td class="py-4">
                                                <div class="fw-bold text-dark">{{ $rapport->titre }}</div>
                                                <div class="small text-muted text-truncate" style="max-width: 300px;">{{ Str::limit($rapport->contenu, 60) }}</div>
                                            </td>
                                            <td class="py-4">
                                                <span class="badge px-3 py-2" style="background-color: #009A4420; color: #009A44; border: 1px solid #009A4440; border-radius: 6px;">
                                                    {{ $rapport->getTypeLabel() }}
                                                </span>
                                            </td>
                                            <td class="py-4">
                                                <div class="d-flex align-items-center">
                                                    ...
                                                    <div>
                                                        <div class="fw-bold small">{{ $rapport->auteur ? $rapport->auteur->prenom . ' ' . $rapport->auteur->nom : 'Non spécifié' }}</div>
                                                        <div class="text-muted extra-small" style="font-size: 0.7rem;">{{ $rapport->auteur?->role?->nom ?? 'Auteur' }}</div>
                                                    </div>
                                                </div>
                                            </td>
                                             <td class="text-end px-4 py-4">
                                                 <div class="d-flex justify-content-end gap-2">
                                                     <button type="button" class="btn btn-sm p-2 d-flex align-items-center justify-content-center" 
                                                             ...
                                                             data-bs-toggle="modal" data-bs-target="#reportModal{{ $rapport->id }}" title="Voir le rapport">
                                                         <i class="bi bi-eye fs-5"></i>
                                                     </button>
                                                     
                                                     <a href="{{ route('partenaire.rapports.voir-pdf', $rapport->id) }}" target="_blank"
                                                     ...
                                                     <a href="{{ route('partenaire.rapports.pdf', $rapport->id) }}" 
                                                        class="btn btn-sm p-2 d-flex align-items-center justify-content-center" 
                                        ...
                                        <div class="modal fade" id="reportModal{{ $rapport->id }}" tabindex="-1" aria-hidden="true">
                                                    ...
                                                    <div class="modal-body p-4">
                                                        <div class="row mb-4">
                                                            <div class="col-md-6">
                                                                ...
                                                                <p class="fw-bold text-dark fs-5">{{ $rapport->titre }}</p>
                                                            </div>
                                                            <div class="col-md-6 text-md-end">
                                                                <h6 class="text-muted small text-uppercase mb-2">Envoyé le</h6>
                                                                <p class="fw-bold text-dark fs-5">{{ $rapport->created_at->format('d/m/Y à H:i') }}</p>
                                                            </div>
                                                        </div>

                                                        <div class="bg-light rounded-4 p-4 mb-4">
                                                            <h6 class="fw-bold text-dark mb-3">Contenu principal</h6>
                                                            <div class="text-muted" style="white-space: pre-wrap;">{{ $rapport->contenu }}</div>
                                                        </div>

                                                        @if($rapport->observations)
                                                        <div class="mb-4">
                                                            <h6 class="fw-bold text-dark mb-2">Observations supplémentaires</h6>
                                                            <p class="text-muted">{{ $rapport->observations }}</p>
                                                        </div>
                                                        @endif
                                                        ...
                                                        <div class="row">
                                                            <div class="col-md-4">
                                                                <div class="p-3 bg-white border rounded-3 text-center">
                                                                    ...
                                                                    <div class="fw-bold fs-4 text-warning">{{ $rapport->avancement_constate }}%</div>
                                                                </div>
                                                            </div>
                                                            <div class="col-md-8">
                                                                <div class="p-3 bg-white border rounded-3">
                                                                    <div class="text-muted small mb-1">Auteur du rapport</div>
                                                                    <div class="fw-bold">{{ $rapport->auteur ? $rapport->auteur->prenom . ' ' . $rapport->auteur->nom : 'Non spécifié' }}</div>
                                                                    <div class="text-muted small">{{ $rapport->auteur?->role?->nom ?? 'Utilisateur' }} &middot; {{ $rapport->projet->nom }}</div>
                                                                </div>
                                                                <div class="p-3 bg-white border rounded-3 mt-2">
                                                                    <div class="text-muted small mb-1">Envoyé par</div>
                                                                    <div class="fw-bold">{{ $rapport->envoyePar ? $rapport->envoyePar->prenom . ' ' . $rapport->envoyePar->nom : $rapport->auteur?->prenom . ' ' . $rapport->auteur?->nom }}</div>
                                                                    <div class="text-muted small">{{ $rapport->envoyePar?->role?->nom ?? ($rapport->auteur?->role?->nom ?? 'Utilisateur') }}</div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
