...
@section('breadcrumb')
    <a href="{{ route('super-admin.configuration.index') }}" class="text-muted">Configuration</a>
    ...
    <div class="cp-content">
        <div class="cp-page-header">
            ...
            <div class="d-flex gap-2">
                <form action="{{ route('super-admin.configuration.logs.clear') }}" method="POST" onsubmit="return confirm('Voulez-vous vraiment vider le fichier de logs ?')">
                    @csrf
                    <button type="submit" class="btn btn-green rounded-pill px-3">
                        <i class="bi bi-x-circle me-1"></i> Vider
                    </button>
                </form>
                <a href="{{ route('super-admin.configuration.logs.export-pdf') }}" class="btn btn-outline-danger rounded-pill px-3">
