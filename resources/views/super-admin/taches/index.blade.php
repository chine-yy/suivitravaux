...
<div class="d-flex gap-2">
                ...
                <a href="{{ route('super-admin.taches.create') }}" class="btn btn-primary px-4">
        ...
        <div class="cp-chart-card mb-4">
            ...
            <div class="p-4">
                <form action="{{ route('super-admin.taches.index') }}" method="GET" class="row g-3">
                    <div class="col-md-3">
                    ...
                    <div class="col-md-2 d-flex align-items-end gap-2">
                        ...
                        <a href="{{ route('super-admin.taches.index') }}" class="btn btn-sm btn-outline-secondary">
        ...
        <div class="cp-chart-card">
            ...
            <div class="table-responsive">
                        ...
                        <tr>
                                ...
                                @if($tache->user)
                                    <div class="d-flex align-items-center gap-2">
                                        <div class="rounded-circle bg-primary text-white d-flex align-items-center justify-content-center fw-bold"
                                            ...
                                            {{ strtoupper(substr($tache->user->prenom ?? $tache->user->name, 0, 1)) }}{{ strtoupper(substr($tache->user->name, 0, 1)) }}
                                        </div>
                                        <div>
                                            <div class="small lh-1">{{ $tache->user->name }} {{ $tache->user->prenom ?? '' }}</div>
                                            @if($tache->user->role)<div class="text-muted" style="font-size:0.6rem;">{{ $tache->user->role->nom }}</div>@endif
                                        </div>
                                    </div>
                                @else
                            ...
                            <td class="text-end">
                                <div class="d-flex gap-1 justify-content-end">
                                    <a href="{{ route('super-admin.taches.show', $tache->id) }}"
                                    ...
                                    <a href="{{ route('super-admin.taches.edit', $tache->id) }}"
                                        class="btn btn-sm btn-outline-primary" title="Modifier">
                                        <i class="bi bi-pencil"></i>
                                    </a>
                                    <form action="{{ route('super-admin.taches.destroy', $tache->id) }}" method="POST"
                                        class="d-inline">
                                        @csrf
                                        @method('DELETE')
                ...
                <button class="btn btn-outline-danger" onclick="exportToPdf('id="tachesTable"', 'Liste des taches', 'taches_export')">                    <i class="bi bi-file-earmark-pdf me-2"></i> Exporter                </button>
                                <a href="{{ route('super-admin.taches.create') }}">Créer une tâche</a>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="text-center py-4 text-muted">
                                            ...
                                            <a href="{{ route('super-admin.sous-taches.create') }}" class="d-block mt-2">Créer une sous-tâche</a>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
