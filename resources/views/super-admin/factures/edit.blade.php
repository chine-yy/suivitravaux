...
@section('breadcrumb')
    <a href="{{ route('super-admin.factures.index') }}" class="text-decoration-none">Factures</a>
...
<div class="cp-dashboard">
    <div class="cp-content">
        <div class="cp-page-header">
            <div>
                <h1 class="cp-page-title"><i class="bi bi-pencil-square me-2"></i>Modifier Facture</h1>
                <p class="cp-page-subtitle">Mettez à jour les informations de la facture</p>
            </div>
            <a href="{{ route('super-admin.factures.index') }}" class="btn btn-outline-secondary">
        ...
        <div class="cp-chart-card">
            ...
            <div class="p-4">
                ...
                @endif
                <form action="{{ route('super-admin.factures.update', $facture->id) }}" method="POST">
                    @csrf @method('PUT')
                    <div class="row g-3">
                        <div class="col-md-6">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="type" class="form-select">
                                ...
                                <option value="avoir" {{ $facture->type == 'avoir' ? 'selected' : '' }}>Avoir</option>
                                <option value="proforma" {{ $facture->type == 'proforma' ? 'selected' : '' }}>Proforma</option>
                            </select>
                        </div>
                        <div class="col-md-6">
                            ...
                            <select name="projet_id" id="projet_id" class="form-select" onchange="updateBudgetRestantFacture(this)">
                                ...
                                <option value="{{ $projet->id }}"
                                    ...
                                    {{ $facture->projet_id == $projet->id ? 'selected' : '' }}>{{ $projet->nom }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="number" step="0.01" name="montant_ht" class="form-control" value="{{ old('montant_ht', $facture->montant_ht) }}">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label fw-semibold">TVA (FCFA)</label>
                            <input type="number" step="0.01" name="montant_tva" class="form-control" value="{{ old('montant_tva', $facture->montant_tva) }}">
                        </div>
                        <div class="col-md-4">
                            ...
                            <input type="number" step="0.01" name="montant_ttc" id="montant_ttc"
                                ...
                                value="{{ old('montant_ttc', $facture->montant_ttc) }}" min="0" inputmode="decimal">
                            @error('montant_ttc') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>
                        <div class="col-md-4">
                            <label class="form-label fw-semibold">Date Émission</label>
                            <input type="date" name="date_emission" class="form-control" value="{{ $facture->date_emission ? date('Y-m-d', strtotime($facture->date_emission)) : '' }}">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label fw-semibold">Date Échéance</label>
                            <input type="date" name="date_echeance" class="form-control" value="{{ $facture->date_echeance ? date('Y-m-d', strtotime($facture->date_echeance)) : '' }}">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label fw-semibold">Statut Paiement</label>
                            <select name="statut_paiement" class="form-select">
                                <option value="en_attente" {{ $facture->statut_paiement == 'en_attente' ? 'selected' : '' }}>En attente</option>
                                <option value="paye" {{ $facture->statut_paiement == 'paye' ? 'selected' : '' }}>Payé</option>
                                <option value="en_retard" {{ $facture->statut_paiement == 'en_retard' ? 'selected' : '' }}>En retard</option>
                                <option value="annule" {{ $facture->statut_paiement == 'annule' ? 'selected' : '' }}>Annulé</option>
                            </select>
                        </div>
                        <div class="col-12">
                            <label class="form-label fw-semibold">Notes</label>
                            <textarea name="notes" class="form-control" rows="3">{{ $facture->notes }}</textarea>
                        </div>
                    </div>
                    <div class="d-flex gap-2 pt-4 border-top">
                        ...
                        <a href="{{ route('super-admin.factures.index') }}" class="btn btn-outline-secondary px-4">Annuler</a>
                    </div>
                </form>
            </div>
        </div>
