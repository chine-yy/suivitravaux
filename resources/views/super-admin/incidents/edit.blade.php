...
<div class="cp-dashboard">
    <div class="cp-content">
        <div class="cp-page-header">
            <div>
                <h1 class="cp-page-title"><i class="bi bi-pencil-square me-2 text-green"></i>Modifier l'Incident</h1>
                <p class="cp-page-subtitle">Mettez à jour les informations de l'incident #{{ $incident->id }}</p>
            </div>
        </div>
        ...
        <div class="cp-chart-card">
            <div class="cp-chart-header">
                <h6 class="cp-chart-title"><i class="bi bi-info-circle me-2 text-green"></i>Détails de l'Incident</h6>
            </div>
            <div class="p-4">
                <form action="{{ route('super-admin.incidents.update', $incident->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row g-4">
                        <div class="col-md-6">
                            ...
                            <select name="projet_id" id="projet_id" class="form-select @error('projet_id') is-invalid @enderror" required>
                                ...
                                @foreach($projets as $projet)
                                    <option value="{{ $projet->id }}" {{ (old('projet_id') ?? $incident->projet_id) == $projet->id ? 'selected' : '' }}>
                                        {{ $projet->nom }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="titre" id="titre" class="form-control @error('titre') is-invalid @enderror"
                                value="{{ old('titre') ?? $incident->titre }}" placeholder="Ex: Retard livraison matériel..." required>
                            @error('titre')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="d-flex flex-wrap gap-4 pt-1">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="gravite" id="gravite_faible" value="faible" {{ (old('gravite') ?? $incident->gravite) == 'faible' ? 'checked' : '' }} required>
                                    <label class="form-check-label text-info fw-medium" for="gravite_faible">
                                        <i class="bi bi-info-circle me-1"></i>Faible
                                    </label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="gravite" id="gravite_moyen" value="moyen" {{ (old('gravite') ?? $incident->gravite) == 'moyen' ? 'checked' : '' }}>
                                    <label class="form-check-label text-warning fw-medium" for="gravite_moyen">
                                        <i class="bi bi-exclamation-triangle me-1"></i>Moyen
                                    </label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="gravite" id="gravite_critique" value="critique" {{ (old('gravite') ?? $incident->gravite) == 'critique' ? 'checked' : '' }}>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="statut" id="statut" class="form-select @error('statut') is-invalid @enderror" required>
                                <option value="ouvert" {{ (old('statut') ?? $incident->statut) == 'ouvert' ? 'selected' : '' }}>Ouvert</option>
                                <option value="en_traitement" {{ (old('statut') ?? $incident->statut) == 'en_traitement' ? 'selected' : '' }}>En cours</option>
                                <option value="resolu" {{ (old('statut') ?? $incident->statut) == 'resolu' ? 'selected' : '' }}>Résolu</option>
                            </select>
                            @error('statut')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="description" id="description" rows="5" class="form-control @error('description') is-invalid @enderror"
                                placeholder="Décrivez précisément l'incident rencontré..." required>{{ old('description') ?? $incident->description }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="d-flex gap-3 pt-3 border-top mt-4 justify-content-end">
                        <a href="{{ route('super-admin.incidents.index') }}" class="btn btn-outline-secondary px-4 py-2">
                            Annuler
                        </a>
                        <button type="submit" class="btn btn-green btn-with-border px-5 py-2 fw-bold shadow-sm">
                            <i class="bi bi-check-circle me-2"></i>Mettre à jour l'Incident
                        </button>
                    </div>
                </form>
            </div>
